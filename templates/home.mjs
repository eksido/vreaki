export function renderHome(site, page, lang, esc) {
  ...
  <header class="site-header" id="top">
    <a class="brand" href="${page.path}" aria-label="${esc(site.name)} — Home"><span>${esc(site.wordmark)}</span><small>${esc(h.tagline)}</small></a>
      ...
      <div class="services-stage">
        ...
        <div class="service-scroller" tabindex="0" role="region" aria-label="${esc(h.nav.services)}">
          <div class="service-rail">${h.services.map((s,i)=>`<article class="service-card tone-${i%4}" id="${s.id}">
            ...
            <a class="service-cta" data-enquiry="${esc(s.label)}" href="${contact(s.label)}">${esc(h.quote)} <span aria-hidden="true">↗</span></a>
          </article>`).join('')}</div>
        </div>
  ...
  <footer id="contact" class="site-footer"><p class="eyebrow">${esc(h.contactLabel)}</p><h2>${esc(h.contactTitle)}</h2><a class="email-link" href="mailto:${esc(site.email)}">${esc(site.email)} <span aria-hidden="true">↗</span></a>
    <form class="enquiry-form" data-recipient="${esc(site.email)}" data-subject="${esc(h.form.subject)}" data-copied="${formText('copied')}" data-copy-error="${esc(h.form.copyError)}" action="mailto:${esc(site.email)}" method="post" enctype="text/plain">
    ...
    <div class="footer-bottom"><span>${esc(site.name)} · ${esc(site.location)}</span><div>${site.socials.map(s=>`<a href="${esc(s.url)}" target="_blank" rel="noopener noreferrer">${esc(s.label)}</a>`).join('')}<a href="#top">${esc(h.backTop)} ↑</a></div></div></footer>`;
}
